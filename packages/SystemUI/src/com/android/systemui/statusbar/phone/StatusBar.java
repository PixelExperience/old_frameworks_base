...
public class StatusBar extends SystemUI implements DemoMode,
    ...
    private AmbientIndicationManagerCallback mAmbientCallback = new AmbientIndicationManagerCallback() {
        ...
        public void onRecognitionResult(Observable observed) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    if ((isKeyguardShowing() || isDozing()) && mRecognitionEnabled && mRecognitionEnabledOnKeyguard){
                        ((AmbientIndicationContainer) mAmbientIndicationContainer).setIndication(observed.Song, observed.Artist);
                        mAmbientIndicationContainer.setVisibility(View.VISIBLE);
                        hideAmbientPlayIndication(mAmbientIndicationManager.getAmbientClearViewInterval());
                    }else{
                        hideAmbientPlayIndication(0);
                    }
                }
        ...
        public void onSettingsChanged(String key, boolean newValue) {
            ...
            }else if (key.contains(Settings.System.AMBIENT_RECOGNITION_KEYGUARD)){
                mRecognitionEnabledOnKeyguard = newValue;
            }
        }
    };

    private void hideAmbientPlayIndication(int interval){
        try {
    ...
    protected void updateKeyguardState(boolean goingToFullShade, boolean fromShadeLocked) {
        ...
        if (mState == StatusBarState.KEYGUARD) {
            ...
            if (mAmbientIndicationContainer != null && mRecognitionEnabled) {
                mAmbientIndicationContainer.setVisibility(View.VISIBLE);
            }
        } else {

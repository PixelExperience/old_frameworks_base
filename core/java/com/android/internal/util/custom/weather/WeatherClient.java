...
public class WeatherClient {
    ...
    private static final int WEATHER_UPDATE_INTERVAL = 60000; // 1 minute, for testing
    private static final String WEATHER_UPDATE_INTENT = "weather_update_action";
    private static final int WEATHER_UPDATE_REQUEST_CODE = 46896087;
    ...
    private BroadcastReceiver weatherReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_SCREEN_OFF.equals(intent.getAction())) {
                ...
                onScreenOn();
            } else if (WEATHER_UPDATE_INTENT.equals(intent.getAction())) {
    ...
    public WeatherClient(Context context) {
        ...
        pendingWeatherUpdate = PendingIntent.getBroadcast(mContext, WEATHER_UPDATE_REQUEST_CODE, new Intent(WEATHER_UPDATE_INTENT), 0);
        mObserver = new ArrayList<>();
        IntentFilter filter = new IntentFilter();
        filter.addAction(Intent.ACTION_SCREEN_OFF);
        filter.addAction(Intent.ACTION_SCREEN_ON);
        filter.addAction(WEATHER_UPDATE_INTENT);
        context.registerReceiver(weatherReceiver, filter);
    }
    ...
    private void updateWeatherAndNotify() {
        ...
        if (DEBUG) Log.d(TAG, "updateWeatherAndNotify");
        isRunning = true;
